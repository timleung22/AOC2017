def Solution(input):
    ...
    for eachRow in input:
        numbers = getNumbers(eachRow)
        checkSum1 += getSumOfQuotients(numbers)
        checkSum2 += getMaxMinDiff(numbers)
...
def getSumOfQuotients(numbers):
    soq = 0
    for i in range(len(numbers)):
        for j in range(len(numbers)):
            if i != j and numbers[i] > numbers[j] and numbers[i] % numbers[j] == 0:
                soq += numbers[i] / numbers[j]

    return soq

def getMaxMinDiff(numbers):
